...
def MFCC(file):
    ...
    num_ceps = 13 # play with me
    num_lifter = 22

    sample_freq, raw_seq = wavfile.read(file)
#    assert raw_seq.ndim == 1  # assume mono
    ...
    afss = np.sum(mfcc_feat[2:], axis=-1) #extracted features
    afss = afss / np.max(np.abs(afss))

    #print(afss)
    return afss

def get_features(string, y, a):
    y1 = y
    path = string
